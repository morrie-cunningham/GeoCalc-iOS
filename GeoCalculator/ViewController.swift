...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var imageLoc1: UIImageView!
    @IBOutlet weak var imageLoc2: UIImageView!
    @IBOutlet weak var tempLoc1: UILabel!
    @IBOutlet weak var tempLoc2: UILabel!
    @IBOutlet weak var weatherLoc1: UILabel!
    @IBOutlet weak var weatherLoc2: UILabel!
    ...
    override func viewDidLoad() {
        ...
        self.tempLoc1.text! = ""
        self.weatherLoc1.text! = ""
        self.tempLoc2.text! = ""
        self.weatherLoc2.text! = ""
    }
    ...
    func doCalculatations()
    {
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p1lt, p1ln)) { (weather) in
            if let w = weather {
                DispatchQueue.main.async {
                    self.imageLoc1.image = UIImage(named: w.iconName)
                    self.tempLoc1.text! = "\(w.temperature)\u{00B0}"
                    self.weatherLoc1.text! = w.summary
                }
            }
        }
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p2lt, p2ln)) { (weather) in
            if let w = weather {
                DispatchQueue.main.async {
                    self.imageLoc2.image = UIImage(named: w.iconName)
                    self.tempLoc2.text! = "\(w.temperature)\u{00B0}"
                    self.weatherLoc2.text! = w.summary
                }
            }
        }
